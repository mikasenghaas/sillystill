...
architecture, and carefully designed loss functions capable of capturing the appearance of film.
...
% Motivation
While unpaired digital or film datasets are abundant, our method requires \textit{paired} digital-film images. This means, that the exact same scene has to be captured with both a digital and a film camera to allow the model to learn differences between the looks of the two mediums. Images should be aligned, as our model should merely transfer style and not move content around, and diverse in order to capture Cinestill-800T's baheviour in a variety of scenes and lighting conditions. As no such dataset was available, we collected our own.

% Dataset collection
\textbf{Data Collection. } To ensure aligned images, we take inspiration from the setup proposed in~\cite{dslr-quality} and utilise two cameras, a Sony Alpha 7 digital camera and a Nikon F3 film camera mounted on a tripod. Photos were taken by keeping the same tripod setup with both cameras. Camera settings, like aperture, ISO, shutter speed, were kept constant across both cameras as best as possible (Table~\ref{tab:cameras}). We only shot static scenes to limit changes in the scene between shots. To create a diverse dataset, images were taken both indoors and outdoors, in different weather conditions, with different light and containing different subjects. Specific emphasis was put on ensuring that all special visual effects (halation, grain and colours) were well-represented in the dataset. The final raw dataset consists of $n=41$ image pairs. Some examples are presented in Figure~\ref{fig:raw-dataset}. 
...
Inevitably, raw image pairs are not perfectly spatially aligned. To address this
