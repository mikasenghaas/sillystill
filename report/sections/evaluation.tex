...
For quantitative evaluation, we use four image quality and image similarity metrics. 
...
learned distance metric based on VGG features, capturing perceptual image
quality based on human-annotated data. Lower LPIPS values indicate higher
...
% Baseline
As our baseline, we compute the above mentioned metrics between the digital and film image in the dataset. Hence, by improving on the baseline our model does indeed learn visual effects that make the predicted film image more similar to the ground truth film image than the model's input.
