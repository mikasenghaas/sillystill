...
quantitative assessments. We qualitatively assess the model's performance by
visually comparing the generated film images to the ground truth film images,
both on patches during training, as well as on the full-sized images at test
time. Beyond qualitative assessment, we evaluate the performance of our model
using four quantitative image quality and image similarity metrics. 
...
does not account for human visual perception and is sensitive to small pixel
variations, often requiring complementary metrics for a fuller quality
assessment.

\textbf{Structural Similarity Index (SSIM)} measures image quality by comparing
...
learned metric based on VGG features, capturing perceptual image quality based
on human-annotated data. LPIPS computes image distance using VGG features,
trained on a dataset where humans chose the most similar image to an original
from distorted images. Lower LPIPS values indicate higher similarity between the
images.
...
% Baseline
As our baseline, we compute the above mentioned metrics between the digital and film image in the dataset. Hence, by improving on the baseline our model does indeed learn visual effects that make the predicted film image more similar to the ground truth film image than the input the model was trained on.
